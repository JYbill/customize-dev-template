...
export class CryptoUtil {
   ...
   */
  static async hashToken256(token: string): Promise<string> {
   ...
   * 存储统一的摘要算法：SHA-256算法
   */
  static async hashStorage256(txt: string): Promise<string> {
    const txtBuf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(txt));
