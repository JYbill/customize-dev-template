...
export class FsUtil {
  ...
  static async move(
    source: string,
    dest: string,
    options: { autoCreateDestFolder?: boolean } = { autoCreateDestFolder: true },
  ) {
   ...
   */
  static async remove(path: string, rmOptions?: { dontThrow: boolean; recursive: boolean }) {
    if (!path) return;
   ...
   * @param path
   */
  static async mkDirRecursive(path: string) {
